...
class ShareService:
    ...
    def create_share(self, *, recipient_email: str, files: list[dict]) -> dict:
        now = self.now_factory()
        expires_at = now + timedelta(seconds=settings.share_ttl_seconds)
        share_id = str(uuid4())
        password = self.secrets_provider.generate_password()
        ...
        upload_urls = [
            {
                ...
                "upload": self.storage.create_upload_url(
                    ...
                    expires_in=min(settings.download_url_ttl_seconds, settings.share_ttl_seconds),
                ),
            }
    ...
    def request_otp(self, *, share_id: str, recipient_email: str, password: str) -> dict:
        ...
        if not self._verify_password(password=password, salt=session.password_salt, expected_hash=session.password_hash):
    ...
    def authorize_download(self, *, share_id: str, recipient_email: str, password: str, otp_code: str) -> dict:
        ...
        if not self._verify_password(password=password, salt=session.password_salt, expected_hash=session.password_hash):
            ...
            raise ValueError("Recipient validation failed")
        now = self.now_factory()
        if not session.otp_code or not session.otp_expires_at or now > session.otp_expires_at:
        ...
        return self.storage.create_zip_download(share_id=share_id, files=session.files, expires_in=settings.download_url_ttl_seconds)
